...
string MakeReceipt(const string *products, const int *prices,
                   const int *quantity, int n, bool discount);
      ...
      while (ProductChoice != 0) {
        ...
        if (ProductChoice == 0) {
          ...
          if (quantity[ProductChoice - 1] + ItemQuantity > maxQuantity) {
            cout << "You can only buy " << maxQuantity << " "
                 << products[ProductChoice - 1] << endl;
            cout << "=============================" << endl;
          } else {
            quantity[ProductChoice - 1] += ItemQuantity;
            cout << "You have added " << ItemQuantity << " "
                 << products[ProductChoice - 1] << endl;
            cout << "=============================" << endl;
          }
        }
...
string MakeReceipt(const string *products, const int *prices,
  ...
  for (int i = 0; i < n; i++) {
    if (quantity[i] > 0) {
      receipt += products[i] + " - " + to_string(quantity[i]) + " - Rp" +
                 to_string(prices[i] * quantity[i]) + "\n";
      if (discount) {
        receipt += "Discount - 10%\n";
...
string ExtractFromReciept(const string &ReceiptVar) {
  ...
  for (int i = 0; i < length; i++) {
    if (ReceiptVar[i] == 'T' && ReceiptVar[i + 1] == 'o' &&
        ReceiptVar[i + 2] == 't' && ReceiptVar[i + 3] == 'a' &&
        ReceiptVar[i + 4] == 'l') {
